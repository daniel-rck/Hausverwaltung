...
export function CostBreakdown() {
  ...
  const availableYears = useMemo(() => {
    ...
    const years = new Set(items.map((i) => parseInt(i.date.slice(0, 4), 10)));
  ...
  const costsByCategory = useMemo(() => {
    ...
    const yearItems = items.filter((i) => parseInt(i.date.slice(0, 4), 10) === effectiveYear);
    const categories: Category[] = ["repair", "maintenance", "inspection", "modernization"];
    const sums = categories.map((cat) =>
      yearItems.filter((i) => i.category === cat).reduce((sum, i) => sum + i.cost, 0),
    );
