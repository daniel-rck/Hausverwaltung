...
import type { Meter, MeterReading, SupplierBill, Unit } from "../../lib/db/schema";
...
export function DifferenzAnalyse({ year }: DifferenzAnalyseProps) {
  ...
  const metersAndReadings = useLiveQuery(async () => {
    ...
    <Card title="Differenzanalyse">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-surface-muted rounded-lg p-3">
          <p className="text-xs text-fg-muted">Versorger gesamt</p>
          <p className="text-lg font-semibold text-fg">{formatNumber(analysis.supplierTotal)} m³</p>
        </div>
        <div className="bg-surface-muted rounded-lg p-3">
          <p className="text-xs text-fg-muted">{messdienstName ?? "Messdienstleister"} gesamt</p>
          <p className="text-lg font-semibold text-fg">
            {formatNumber(analysis.messdienstTotal)} m³
          </p>
        </div>
        <div className="bg-surface-muted rounded-lg p-3">
          <p className="text-xs text-fg-muted">Differenz</p>
          <p className="text-lg font-semibold text-fg">
            {formatNumber(analysis.difference)} m³ ({formatNumber(analysis.differencePercent)} %)
          </p>
        </div>
        <div className="bg-surface-muted rounded-lg p-3 flex flex-col justify-center">
          <p className="text-xs text-fg-muted mb-1">Bewertung</p>
          <StatusBadge status={analysis.status} label={analysis.statusLabel} />
        </div>
      </div>

      <p className="text-xs text-fg-muted mb-2">
        Schwellwerte: &lt;{WATER_DIFF_THRESHOLD_OK} % = im Rahmen, {WATER_DIFF_THRESHOLD_OK}–
        {WATER_DIFF_THRESHOLD_WARN} % = auffällig, &gt;{WATER_DIFF_THRESHOLD_WARN} % = kritisch
      </p>

      {chartData && (
        <div className="mt-4">
          <BarChart labels={chartData.labels} datasets={chartData.datasets} height={220} />
        </div>
      )}
    </Card>
  );
