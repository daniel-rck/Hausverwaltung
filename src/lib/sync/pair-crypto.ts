...
async function deriveKey(otp: string, salt: Uint8Array<ArrayBuffer>): Promise<CryptoKey> {
  const ikm = await crypto.subtle.importKey("raw", utf8Bytes(otp), "HKDF", false, ["deriveKey"]);
...
export async function wrapSecretForPairing(secret: string, otp: string): Promise<WrappedPayload> {
