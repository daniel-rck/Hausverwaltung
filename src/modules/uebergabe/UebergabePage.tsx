...
import { createDefaultRooms, RoomInspection } from "./RoomInspection";
import { SignatureCanvas } from "./SignatureCanvas";
import type { MeterDetail, ProtocolContext } from "./UebergabePrint";
...
export function UebergabePage() {
  ...
  const protocols = useLiveQuery(async (): Promise<
    (HandoverProtocol & { tenantName: string; unitName: string })[]
  > => {
    if (!activeProperty?.id) return [];
                ...
                >
                  ...
                  {occupancies?.map((o) => (
                    <option key={o.occupancy.id} value={o.occupancy.id}>
                      {o.unit.name} – {o.tenant?.name ?? "Unbekannt"} ({o.occupancy.from} bis{" "}
                      {o.occupancy.to ?? "heute"})
                    </option>
                  ))}
                </Select>
              </FormField>
