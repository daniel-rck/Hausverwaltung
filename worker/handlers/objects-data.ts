import { jsonError, verifyBearerForId } from "../lib/auth";
...
export async function handleObjectGet(request: Request, env: Env, id: string): Promise<Response> {
...
export async function handleObjectPut(request: Request, env: Env, id: string): Promise<Response> {
  const ip = clientIp(request);
  const rl = await rateLimit(env, `data:${ip}`, 60, 60);
  if (!rl.allowed) return rateLimited(rl.retryAfter);
